//
//  BxlMPosControllerDevicePlugin.m
//  bxlflutterbgatelib
//
//  Created by OhDonggeon on 2023/11/01.
//
...
static NSString* const PLATFORM_CHANNEL = @"com.bixolon.mposcontroller/device";
static NSString* const DATA_EVENT_CHANNEL = @"com.bixolon.mposcontroller/device/data";

@interface DeviceDataEvent : NSObject<FlutterStreamHandler>

@property(weak) BxlMPosControllerDevicePlugin* plugin;
@property FlutterEventSink eventSink;

@end

@implementation DeviceDataEvent

- (id)initWithPlugin:(BxlMPosControllerDevicePlugin*)plugin{
    if((self=[super init])){
        self.plugin = plugin;
...
@interface BxlMPosControllerDevicePlugin()
@property MPosControllerDevices* devices;
@end

@implementation BxlMPosControllerDevicePlugin

+ (void)registerWithRegistrar:(nonnull NSObject<FlutterPluginRegistrar> *)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
                                     methodChannelWithName: PLATFORM_CHANNEL
                                     binaryMessenger:[registrar messenger]];
    BxlMPosControllerDevicePlugin* instance = [[BxlMPosControllerDevicePlugin alloc] init];
    ...
    [eventChannel setStreamHandler:[[DeviceDataEvent alloc] initWithPlugin:instance]];
}

- (instancetype)init {
    if (self = [super init]) {
        _devices = [MPosControllerDevices new];
    }
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"openService" isEqualToString:call.method])
        ...
        return [self getSdkVersion:call result:result];
...
- (void)openService:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.devices openService:[deviceId integerValue] timeout:[timeout integerValue]];
...
- (void)closeService:(FlutterMethodCall*)call result:(FlutterResult)result {
    NSDictionary* arguments = [call arguments];
    NSNumber* timeout = arguments[@"time_out"] ? : @3;
    MPOS_RESULT nativeResult = [self.devices closeService:[timeout integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectInterface:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.devices selectInterface:[interfaceType integerValue] address:address];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectCommandMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.devices selectCommandMode:[commandMode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setTransaction:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.devices setTransaction:[transaction integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setReadMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.devices setReadMode:[mode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)isUsbPeripheralPrinter:(FlutterMethodCall*)call result:(FlutterResult)result {
    result([self.devices isUSBPeripheralPrinter] ? @(YES) : @(NO));
}

- (void)getSdkVersion:(FlutterMethodCall*)call result:(FlutterResult)result {
    result([self.devices getSDKVersion]);
}

- (NSInteger)getDeviceId {
    return [self.devices getDeviceId];
}
