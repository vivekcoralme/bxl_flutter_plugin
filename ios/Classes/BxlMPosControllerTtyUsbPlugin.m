//
//  BxlMPosControllerTtyUsbPlugin.m
//  bxlflutterbgatelib
//
...
static NSString* const PLATFORM_CHANNEL = @"com.bixolon.mposcontroller/usbtoserial";
static NSString* const DATA_EVENT_CHANNEL = @"com.bixolon.mposcontroller/usbtoserial/data";

@interface TtyUsbDataEvent : NSObject<FlutterStreamHandler>

@property(weak) BxlMPosControllerTtyUsbPlugin* plugin;
@property FlutterEventSink eventSink;

@end

@implementation TtyUsbDataEvent

- (id)initWithPlugin:(BxlMPosControllerTtyUsbPlugin*)plugin{
    if((self=[super init])){
        self.plugin = plugin;
...
@interface BxlMPosControllerTtyUsbPlugin()
@property MPosControllerUSBSerial* ttyUsb;
@end

@implementation BxlMPosControllerTtyUsbPlugin

+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
                                     methodChannelWithName: PLATFORM_CHANNEL
                                     binaryMessenger:[registrar messenger]];
    BxlMPosControllerTtyUsbPlugin* instance = [[BxlMPosControllerTtyUsbPlugin alloc] init];
    ...
    [eventChannel setStreamHandler:[[TtyUsbDataEvent alloc] initWithPlugin:instance]];
}

- (id)init {
    if ((self = [super init])) {
        self.ttyUsb = [MPosControllerUSBSerial new];
    }
    
    [MPosControllerUSBSerial setEnableLog:YES saveFile:NO saveToHex:NO];
...
- (void)openService:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.ttyUsb openService:[deviceId integerValue] timeout:[timeout integerValue]];
...
- (void)closeService:(FlutterMethodCall*)call result:(FlutterResult)result {
    NSDictionary* arguments = [call arguments];
    NSNumber* timeout = arguments[@"time_out"] ? : @3;
    MPOS_RESULT nativeResult = [self.ttyUsb closeService:[timeout integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectInterface:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.ttyUsb selectInterface:[interfaceType integerValue] address:address];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectCommandMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.ttyUsb selectCommandMode:[commandMode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setTransaction:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.ttyUsb setTransaction:[transaction integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setReadMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.ttyUsb setReadMode:[mode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)isUsbPeripheralPrinter:(FlutterMethodCall*)call result:(FlutterResult)result {
    result([self.ttyUsb isUSBPeripheralPrinter] ? @(YES) : @(NO));
}

- (NSInteger)getDeviceId {
    return [self.ttyUsb getDeviceId];
}
