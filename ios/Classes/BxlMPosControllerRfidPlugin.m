...
static NSString* const PLATFORM_CHANNEL = @"com.bixolon.mposcontroller/rfid";
static NSString* const DATA_EVENT_CHANNEL = @"com.bixolon.mposcontroller/ttyusb/data";

@interface RfidDataEvent : NSObject<FlutterStreamHandler>

@property(weak) BxlMPosControllerRfidPlugin* plugin;
@property FlutterEventSink eventSink;

@end

@implementation RfidDataEvent

- (id)initWithPlugin:(BxlMPosControllerRfidPlugin*)plugin{
    if((self=[super init])){
        self.plugin = plugin;
...
@interface BxlMPosControllerRfidPlugin()
@property MPosControllerRFID* rfid;
@end

@implementation BxlMPosControllerRfidPlugin

+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
                                     methodChannelWithName: PLATFORM_CHANNEL
                                     binaryMessenger:[registrar messenger]];
    BxlMPosControllerRfidPlugin* instance = [[BxlMPosControllerRfidPlugin alloc] init];
    ...
    [eventChannel setStreamHandler:[[RfidDataEvent alloc] initWithPlugin:instance]];
}

- (id)init {
    if ((self = [super init])) {
        self.rfid = [MPosControllerRFID new];
    }
    
    [MPosControllerRFID setEnableLog:YES saveFile:NO saveToHex:NO];
...
- (void)openService:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.rfid openService:[deviceId integerValue] timeout:[timeout integerValue]];
...
- (void)closeService:(FlutterMethodCall*)call result:(FlutterResult)result {
    NSDictionary* arguments = [call arguments];
    NSNumber* timeout = arguments[@"time_out"] ? : @3;
    MPOS_RESULT nativeResult = [self.rfid closeService:[timeout integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectInterface:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.rfid selectInterface:[interfaceType integerValue] address:address];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectCommandMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.rfid selectCommandMode:[commandMode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setTransaction:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.rfid setTransaction:[transaction integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setReadMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.rfid setReadMode:[mode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)directIO:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.rfid directIO:dataToSend];
    result([NSNumber numberWithInteger:nativeResult]);
}

- (void)isOpen:(FlutterMethodCall*)call result:(FlutterResult)result {
    MPOS_RESULT nativeResult = [self.rfid isOpen] ? 1 : 0;
    result([NSNumber numberWithInteger:nativeResult]);
}

- (void)getDeviceId:(FlutterMethodCall*)call result:(FlutterResult)result {
    MPOS_RESULT nativeResult = [self.rfid getDeviceId];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setKeepNetworkConnection:(FlutterMethodCall*)call result:(FlutterResult)result {
    if (![self.rfid isOpen]) {
        ...
        [self.rfid setKeepNetworkConnection:[keep_connection integerValue]];
...
- (void)isUsbPeripheralPrinter:(FlutterMethodCall*)call result:(FlutterResult)result {
    result([self.rfid isUSBPeripheralPrinter] ? @(YES) : @(NO));
}

- (NSInteger)getDeviceId {
    return [self.rfid getDeviceId];
}
