//
//  BxlMPosControllerDallasKeyPlugin.m
//  bxlflutterbgatelib
//
...
static NSString* const PLATFORM_CHANNEL = @"com.bixolon.mposcontroller/dallaskey";
static NSString* const DATA_EVENT_CHANNEL = @"com.bixolon.mposcontroller/ttyusb/data";

@interface DallasKeyDataEvent : NSObject<FlutterStreamHandler>

@property(weak) BxlMPosControllerDallasKeyPlugin* plugin;
@property FlutterEventSink eventSink;

@end

@implementation DallasKeyDataEvent

- (id)initWithPlugin:(BxlMPosControllerDallasKeyPlugin*)plugin{
    if((self=[super init])){
        self.plugin = plugin;
...
@interface BxlMPosControllerDallasKeyPlugin()
@property MPosControllerDallasKey* dallasKey;
@end

@implementation BxlMPosControllerDallasKeyPlugin

+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
                                     methodChannelWithName: PLATFORM_CHANNEL
                                     binaryMessenger:[registrar messenger]];
    BxlMPosControllerDallasKeyPlugin* instance = [[BxlMPosControllerDallasKeyPlugin alloc] init];
    ...
    [eventChannel setStreamHandler:[[DallasKeyDataEvent alloc] initWithPlugin:instance]];
}

- (id)init {
    if ((self = [super init])) {
        self.dallasKey = [MPosControllerDallasKey new];
    }
    
    [MPosControllerDallasKey setEnableLog:YES saveFile:NO saveToHex:NO];
...
- (void)openService:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.dallasKey openService:[deviceId integerValue] timeout:[timeout integerValue]];
...
- (void)closeService:(FlutterMethodCall*)call result:(FlutterResult)result {
    NSDictionary* arguments = [call arguments];
    NSNumber* timeout = arguments[@"time_out"] ? : @3;
    MPOS_RESULT nativeResult = [self.dallasKey closeService:[timeout integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectInterface:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.dallasKey selectInterface:[interfaceType integerValue] address:address];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectCommandMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.dallasKey selectCommandMode:[commandMode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setTransaction:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.dallasKey setTransaction:[transaction integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setReadMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.dallasKey setReadMode:[mode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)isUsbPeripheralPrinter:(FlutterMethodCall*)call result:(FlutterResult)result {
    result([self.dallasKey isUSBPeripheralPrinter] ? @(YES) : @(NO));
}

- (NSInteger)getDeviceId {
    return [self.dallasKey getDeviceId];
}
