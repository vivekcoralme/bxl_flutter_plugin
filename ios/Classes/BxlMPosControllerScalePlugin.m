//
//  BxlMPosControllerScalePlugin.m
//  bxlflutterbgatelib
//
...
static NSString* const PLATFORM_CHANNEL = @"com.bixolon.mposcontroller/scale";
static NSString* const DATA_EVENT_CHANNEL = @"com.bixolon.mposcontroller/ttyusb/data";

@interface ScaleDataEvent : NSObject<FlutterStreamHandler>

@property(weak) BxlMPosControllerScalePlugin* plugin;
@property FlutterEventSink eventSink;

@end

@implementation ScaleDataEvent

- (id)initWithPlugin:(BxlMPosControllerScalePlugin*)plugin{
    if((self=[super init])){
        self.plugin = plugin;
...
@interface BxlMPosControllerScalePlugin()
@property MPosControllerScale* scale;
@end

@implementation BxlMPosControllerScalePlugin
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    ...
    BxlMPosControllerScalePlugin* instance = [[BxlMPosControllerScalePlugin alloc] init];
    ...
    [eventChannel setStreamHandler:[[ScaleDataEvent alloc] initWithPlugin:instance]];
}

- (id)init {
    if ((self = [super init])) {
        self.scale = [MPosControllerScale new];
    }
    
    [MPosControllerScale setEnableLog:YES saveFile:NO saveToHex:NO];
...
- (void)openService:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.scale openService:[deviceId integerValue] timeout:[timeout integerValue]];
...
- (void)closeService:(FlutterMethodCall*)call result:(FlutterResult)result {
    NSDictionary* arguments = [call arguments];
    NSNumber* timeout = arguments[@"time_out"] ? : @3;
    MPOS_RESULT nativeResult = [self.scale closeService:[timeout integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectInterface:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.scale selectInterface:[interfaceType integerValue] address:address];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)selectCommandMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.scale selectCommandMode:[commandMode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setTransaction:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.scale setTransaction:[transaction integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)setReadMode:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    MPOS_RESULT nativeResult = [self.scale setReadMode:[mode integerValue]];
    result([NSNumber numberWithInteger:nativeResult]);
}
...
- (void)isUsbPeripheralPrinter:(FlutterMethodCall*)call result:(FlutterResult)result {
    result([self.scale isUSBPeripheralPrinter] ? @(YES) : @(NO));
}

- (NSInteger)getDeviceId {
    return [self.scale getDeviceId];
}
